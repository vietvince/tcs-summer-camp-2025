...
- Walk through a loop and how they are used to repeat actions until a condition is met.
- How conditional statements execute tasks based on the requirements.
- Recongize how Python uses indentation to define blocks of code.
- How to use `undo()` to reverse actions and `clear()` to clean the drawing screen.
...
- Iterable objects: the concept, example with `enumerate()`.
- List comphresions and integrating them with conditional logic.
- Varying implementions of `while` loops, such as `while True` with a `break` statement versus using a control variable versus using a condition statement.
- Multiple conditions with logical operators (`and`, `or`, `not`).
- Error handling with statements such as `try`, `except`, and `finally`.
- Stateful programming versus stateless programming.
- A basic overview of data structures like stacks that are commonly used to keep track of past states and actions for undo operations.
...
<div align="center">
    ![nerdEmoji](./../../static/img/py1/py1i.png)
</div>
...
<div align="center">
    ![nerdEmoji](./../../static/img/py1/py1w.gif)
</div>
